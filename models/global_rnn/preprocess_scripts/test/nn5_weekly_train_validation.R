...
output_file_prefix <- "nn5_weekly_"
k_val <- 5
input_size <- 10
...
for (validation in c(TRUE, FALSE)) {
    ...
    for (idr in 1 : length(df_train)) {
        ...
        for(val in sin_indexes){
          ...
          sin_windows <- embed(sin_terms[1 : (time_series_length - max_forecast_horizon)], input_size)[, input_size : 1]
          cos_windows <- embed(cos_terms[1 : (time_series_length - max_forecast_horizon)], input_size)[, input_size : 1]
        ...
        if (validation) {
            sav_df <- matrix(NA, ncol = (11 + input_size + 4 + max_forecast_horizon), nrow = nrow(input_windows))
            ...
            sav_df[, (input_size + 10 + 2)] <- '|o'
            sav_df[, (input_size + 10 + 3) : (input_size + 10 + max_forecast_horizon + 2)] <- output_windows
            sav_df[, (input_size + 10 + max_forecast_horizon + 3)] <- '|#'
            sav_df[, (input_size + 10 + max_forecast_horizon + 4)] <- time_series_mean
            sav_df[, (input_size + 10 + max_forecast_horizon + 5)] <- meanvalues
        }else {
            sav_df <- matrix(NA, ncol = (11 + input_size + 1 + max_forecast_horizon), nrow = nrow(input_windows))
            sav_df <- as.data.frame(sav_df)
            sav_df[, 1] <- paste(idr - 1, '|i', sep = '')
            sav_df[, 2] <- seasonality_sin_windows_1
            sav_df[, 3] <- seasonality_cos_windows_2
