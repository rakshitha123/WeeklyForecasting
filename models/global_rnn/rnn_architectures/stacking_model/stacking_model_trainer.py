...
from models.global_rnn.tfrecords_handler.moving_window.tfrecord_reader import TFRecordReader
from models.global_rnn.configs.global_configs import gpu_configs
...
class StackingModelTrainer:
    ...
    def train_model(self, return_dict, kwargs):
            ...
            while True:
                try:
                    ...
                    if self.__without_stl_decomposition:
                        ...
                        converted_validation_output = np.exp(level_values[:, np.newaxis] + last_validation_outputs)
                        converted_actual_values = np.exp(level_values[:, np.newaxis] + actual_values)
        ...
        return_dict[0] = float(smape_final)
        return_dict[1] = smape_list
